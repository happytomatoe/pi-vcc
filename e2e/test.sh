#!/usr/bin/env bash
# E2E test: verify pi-vcc compaction triggers and agent continues
# Uses openrouter/free (no API credits needed)
# Max 180 seconds total
set -u
...
mkdir -p "$CAST_DIR"
trap '"$SU" --session "$SESSION" get-recording >"$CAST_DIR/test-threshold.cast" 2>/dev/null; cleanup' EXIT
